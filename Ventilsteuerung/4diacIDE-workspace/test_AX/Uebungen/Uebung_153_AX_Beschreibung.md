...
In landtechnischen Anwendungen (z. B. Säwellenantrieb, Düngerstreuer-Dosierscheibe, Lüfterdrehzahlsteuerung) muss eine Drehzahl oder Impulsfrequenz gemessen und über einen PID-Regler (`CTRL_PID_AR`) auf einen Sollwert geregelt werden. Die Stellgröße wird als PWM-Signal an einem digitalen Leistungsausgang (`Output_Q1`) ausgegeben.
...
- **Regelung:** Der PID-Regler-Adapterwrapper `CTRL_PID_AR` vergleicht den Istwert (`AR_ACT`) mit dem Sollwert (`SET = 16.0`). Über Taster `I1` (via `AX_T_FF.Q`) wird direkt über den Adapter-Socket `AX_MAN` zwischen Automatik- und Handbetrieb umgeschaltet.
