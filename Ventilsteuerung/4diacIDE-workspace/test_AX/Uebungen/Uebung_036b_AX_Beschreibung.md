...
6. Zur Visualisierung des aktuellen Schrittes soll die Zustandsnummer `STATE_NR` über den Baustein `AS_TO_AUDI` in einen `UDINT`-Wert (AUDI) konvertiert und an den ISOBUS Virtual Terminal-Ausgang `OutputNumber_N1` gesendet werden.
