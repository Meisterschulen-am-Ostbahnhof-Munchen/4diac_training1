...
Variante von [Übung 225](Uebung_225_Beschreibung.md): dieselbe Aufgabe, aber statt die drei
...
analog zum Mittelmarker, wie er in vergleichbaren Anwendungen für Achslenkung/Hangausgleich
...
1. Legen Sie die SubApp `Uebung_225b` an (bereits als Referenzlösung vorhanden).
...
`Uebung_225b.SUB` — validiert gegen `subapptype.xsd` (siehe `iec61499-creator`-Skill). Für die
vollständig über Adapter verdrahtete Variante siehe `Uebung_225b_AX` in `test_AX`. Der
