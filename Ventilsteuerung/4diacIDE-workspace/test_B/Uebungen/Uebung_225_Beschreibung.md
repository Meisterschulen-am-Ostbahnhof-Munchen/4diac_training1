...
1. Legen Sie die SubApp `Uebung_225` an (bereits als Referenzlösung vorhanden).
...
`Uebung_225.SUB` — validiert gegen `fbtype.xsd`/`subapp.xsd` (siehe `iec61499-creator`-Skill). Für
die vollständig über Adapter verdrahtete Variante siehe `Uebung_225_AX` in `test_AX`.
