// ISO-Designer ISO 11783   Version 5.7.2.6664 Bucher Automation AG
// Do not change!

#include "DefaultPool.iop.h"
#include "DefaultPool.c.h"

#define WORD(w)  (unsigned char)w, (unsigned char)(w >> 8)
#define LONG(l)  (unsigned char)l, \
					(unsigned char)((unsigned long)l >> 8), \
					(unsigned char)((unsigned long)l >> 16), \
					(unsigned char)((unsigned long)l >> 24)
#define ID(id)           WORD(id)
#define REF(id)          WORD(id)
#define XYREF(id, x, y)  WORD(id), WORD(x), WORD(y)
#define MACRO(ev, id)    ev, id
#define COLOR(c)         c

const unsigned char ISO_OP_MEMORY_CLASS isoOP_DefaultPool[] = {
	17,
	ID(WorkingSet), TYPEID_WORKSET, COLOR_SILVER, 1, ID(DataMask_M1), 1, 0, 2,
		XYREF(Line, 40, 40),
		'e', 'n',
		'd', 'e',
	ID(DataMask_M1), TYPEID_DATAMASK, COLOR_SILVER, ID(ID_NULL), 5, 0, 
		XYREF(Battery_disconnect, 40, 200),
		XYREF(Container_Wheel_based_machine_speed, 40, 40),
		XYREF(Container_Ground_based_machine_speed, 40, 280),
		XYREF(Container_Wheel_based_machine_distance, 40, 120),
		XYREF(Container_Rear_PTO_output_shaft_speed, 40, 360),
	ID(Container_Wheel_based_machine_speed), TYPEID_CONTAINER, WORD(362), WORD(64), 0, 5, 0, 
		XYREF(Tractor_wheel_slip, 0, 0),
		XYREF(OutputNumber_Wheel_based_machine_speed_km_h, 80, 0),
		XYREF(OutputNumber_Wheel_based_machine_speed_m_s, 80, 32),
		XYREF(OutputString_km_h, 282, 0),
		XYREF(OutputString_m_s, 282, 32),
	ID(Container_Ground_based_machine_speed), TYPEID_CONTAINER, WORD(362), WORD(64), 0, 5, 0, 
		XYREF(Tractor_ground_speed, 0, 0),
		XYREF(OutputNumber_Ground_based_machine_speed_km_h, 80, 0),
		XYREF(OutputNumber_Ground_based_machine_speed_m_s, 80, 32),
		XYREF(OutputString_km_h, 282, 0),
		XYREF(OutputString_m_s, 282, 32),
	ID(Container_Wheel_based_machine_distance), TYPEID_CONTAINER, WORD(362), WORD(64), 0, 5, 0, 
		XYREF(Work_distance_travelled, 0, 0),
		XYREF(OutputNumber_Wheel_based_machine_distance_km, 80, 0),
		XYREF(OutputNumber_Wheel_based_machine_distance_m, 80, 32),
		XYREF(OutputString_km, 282, 0),
		XYREF(OutputString_m, 282, 32),
	ID(Container_Rear_PTO_output_shaft_speed), TYPEID_CONTAINER, WORD(362), WORD(64), 0, 3, 0, 
		XYREF(PTO_output_shaft_speed, 0, 0),
		XYREF(OutputNumber_Rear_PTO_output_shaft_speed, 80, 0),
		XYREF(OutputString_1_min, 282, 0),
	ID(SoftKeyMask_S1), TYPEID_SKEYMASK, COLOR_SILVER, 0, 0, 
	ID(OutputString_km_h), TYPEID_OUTSTR, WORD(80), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputString), 0,
		ID(ID_NULL), 0, WORD(4), 'k', 'm', '/', 'h', 0,
	ID(OutputString_m_s), TYPEID_OUTSTR, WORD(80), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputString), 0,
		ID(ID_NULL), 0, WORD(3), 'm', '/', 's', 0,
	ID(OutputString_km), TYPEID_OUTSTR, WORD(80), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputString), 0,
		ID(ID_NULL), 0, WORD(2), 'k', 'm', 0,
	ID(OutputString_m), TYPEID_OUTSTR, WORD(80), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputString), 0,
		ID(ID_NULL), 0, WORD(1), 'm', 0,
	ID(OutputString_1_min), TYPEID_OUTSTR, WORD(80), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputString), 0,
		ID(ID_NULL), 0, WORD(5), '1', '/', 'm', 'i', 'n', 0,
	ID(OutputNumber_Wheel_based_machine_speed_km_h), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_Wheel_based_machine_speed), LONG(0UL), LONG(0L), 0xFA, 0xED, 0x6B, 0x3B, 1, 0, 0, 0,
	ID(OutputNumber_Wheel_based_machine_speed_m_s), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_Wheel_based_machine_speed), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_Ground_based_machine_speed_km_h), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_Ground_based_machine_speed), LONG(0UL), LONG(0L), 0xFA, 0xED, 0x6B, 0x3B, 1, 0, 0, 0,
	ID(OutputNumber_Ground_based_machine_speed_m_s), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_Ground_based_machine_speed), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_Wheel_based_machine_distance_km), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_Wheel_based_machine_distance), LONG(0UL), LONG(0L), 0xBD, 0x37, 0x86, 0x35, 1, 0, 0, 0,
	ID(OutputNumber_Wheel_based_machine_distance_m), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_Wheel_based_machine_distance), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_Rear_PTO_output_shaft_speed), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_Rear_PTO_output_shaft_speed), LONG(0UL), LONG(0L), 0x00, 0x00, 0x00, 0x3E, 1, 0, 0, 0,
	ID(OutputNumber_TECU_Speed_WBSD), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_Speed_WBSD), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_TECU_Speed_GBSD), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_Speed_GBSD), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_TECU_Speed_MSS), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_Speed_MSS), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_TECU_Speed_VDS), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_Speed_VDS), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_TECU_Speed_COGSOG), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_Speed_COGSOG), LONG(0UL), LONG(0L), FLOAT_0_001, 1, 0, 0, 0,
	ID(OutputNumber_TECU_FHS_Position), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_FHS_Position), LONG(0UL), LONG(0L), 0xCD, 0xCC, 0xCC, 0x3E, 1, 0, 0, 0,
	ID(OutputNumber_TECU_RHS_Position), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_RHS_Position), LONG(0UL), LONG(0L), 0xCD, 0xCC, 0xCC, 0x3E, 1, 0, 0, 0,
	ID(OutputNumber_TECU_FHS_Force), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_FHS_Force), LONG(0UL), LONG(0L), FLOAT_1, 0, 0, 0, 0,
	ID(OutputNumber_TECU_RHS_Force), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_RHS_Force), LONG(0UL), LONG(0L), FLOAT_1, 0, 0, 0, 0,
	ID(OutputNumber_TECU_FHS_Draft), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_FHS_Draft), LONG(0UL), LONG(0L), FLOAT_1, 0, 0, 0, 0,
	ID(OutputNumber_TECU_RHS_Draft), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_RHS_Draft), LONG(0UL), LONG(0L), FLOAT_1, 0, 0, 0, 0,
	ID(OutputNumber_TECU_FPTO_Speed), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_FPTO_Speed), LONG(0UL), LONG(0L), 0x00, 0x00, 0x00, 0x3E, 1, 0, 0, 0,
	ID(OutputNumber_TECU_RPTO_Speed), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_RPTO_Speed), LONG(0UL), LONG(0L), 0x00, 0x00, 0x00, 0x3E, 1, 0, 0, 0,
	ID(OutputNumber_TECU_FPTO_SpeedSet), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_FPTO_SpeedSet), LONG(0UL), LONG(0L), 0x00, 0x00, 0x00, 0x3E, 1, 0, 0, 0,
	ID(OutputNumber_TECU_RPTO_SpeedSet), TYPEID_OUTNUM, WORD(200), WORD(30), COLOR_WHITE, ID(FontAttributes_OutputNumber), 0,
		ID(NumberVariable_TECU_RPTO_SpeedSet), LONG(0UL), LONG(0L), 0x00, 0x00, 0x00, 0x3E, 1, 0, 0, 0,
	ID(Line), TYPEID_OUTLINE, ID(LineAttributes), WORD(40), WORD(1), 0, 0,
	ID(Tractor_wheel_slip), TYPEID_OUTPICT, WORD(64), WORD(64), WORD(64), 
		0, 0, COLOR_MAGENTA, LONG(512UL), 0, 
		, 
	ID(Battery_disconnect), TYPEID_OUTPICT, WORD(64), WORD(64), WORD(64), 
		0, 0, COLOR_MAGENTA, LONG(512UL), 0, 
		, 
	ID(Work_distance_travelled), TYPEID_OUTPICT, WORD(64), WORD(64), WORD(64), 
		0, 0, COLOR_MAGENTA, LONG(512UL), 0, 
		, 
	ID(Tractor_ground_speed), TYPEID_OUTPICT, WORD(64), WORD(64), WORD(64), 
		0, 0, COLOR_MAGENTA, LONG(512UL), 0, 
		, 
	ID(PTO_output_shaft_speed), TYPEID_OUTPICT, WORD(64), WORD(64), WORD(64), 
		0, 0, COLOR_MAGENTA, LONG(512UL), 0, 
		, 
	ID(NumberVariable_Wheel_based_machine_speed), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_Ground_based_machine_speed), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_Wheel_based_machine_distance), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_Rear_PTO_output_shaft_speed), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_Speed_WBSD), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_Speed_GBSD), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_Speed_MSS), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_Speed_VDS), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_Speed_COGSOG), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_FHS_Position), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_RHS_Position), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_FHS_Force), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_RHS_Force), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_FHS_Draft), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_RHS_Draft), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_FPTO_Speed), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_RPTO_Speed), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_FPTO_SpeedSet), TYPEID_VARNUM, LONG(0UL), 
	ID(NumberVariable_TECU_RPTO_SpeedSet), TYPEID_VARNUM, LONG(0UL), 
	ID(FontAttributes_OutputNumber), TYPEID_FONTATTR, COLOR_BLACK, 3, 0, 0, 0, 
	ID(FontAttributes_OutputString), TYPEID_FONTATTR, COLOR_BLACK, 3, 0, 0, 0, 
	ID(LineAttributes), TYPEID_LINEATTR, COLOR_BLACK, 1, WORD(65535), 0, 
}; // isoOP_DefaultPool
