...
def parse_fb_positions(text: str) -> Dict[str, Tuple[float, float]]:
    """Return mapping of FB instance name -> (x, y) parsed from the
    <SubAppNetwork> FB declarations. FBs declared without x/y are placed
    at (0, 0) so the layout helper still produces stable coordinates."""
    pos: Dict[str, Tuple[float, float]] = {}
    for m in re.finditer(
        r'<FB\s+Name="([^"]+)"[^/>]*?\bx="(-?[\d.]+)"\s+y="(-?[\d.]+)"',
        text,
    ):
        try:
            pos[m.group(1)] = (float(m.group(2)), float(m.group(3)))
        except ValueError:
            pos[m.group(1)] = (0.0, 0.0)
    return pos
...
def _fmt_coord(v: float) -> str:
    """Render a coordinate matching 4diac IDE style: integers without a
    decimal point, everything else with up to two decimals (no trailing
    zeros)."""
    iv = int(round(v))
    if abs(v - iv) < 1e-6:
        return str(iv)
...
def build_split_chain(existing_fb_names: set, n: int,
                      start_x: float, y: float) -> Tuple[List[Tuple[str, str, int, float, float]], int]:
    """Build a list of E_SPLIT blocks whose combined fan-out == n.
    Each entry: (instance_name, type, fan_out_of_this_block, x, y).
    The chain is laid out left-to-right starting at start_x; multiple
    chain elements share the same y so the user sees a clean row."""
    blocks = []
    remaining = n
    x = start_x
    while remaining > 1:
        ...
        blocks.append((bname, btype, k, x, y))
        x += 800
...
def process_subapp_text(subapp_text: str) -> Tuple[str, int]:
    ...
    # Existing FB positions for layout of new split blocks
    fb_positions = parse_fb_positions(subapp_text)

    by_src = group_by_source(conns)
    plan: List[dict] = []
    # plan entry: { "src": (fb, port), "dests": [orig_conn_indices...],
    #               "chain": [(bname, btype, k, x, y), ...] }
    ...
    for (fbname, port), idxs in by_src.items():
        ...
        if any(t.endswith(".EI") and t.split(".")[0].startswith("E_SPLIT") for t in targets):
            continue
        # Place the split chain just left of the source FB so it stays
        # visually close to the source.
        src_x, src_y = fb_positions.get(fbname, (0.0, 0.0))
        chain, leftover = build_split_chain(fb_names, len(idxs),
                                            start_x=src_x - 800, y=src_y)
    ...
    for p in plan:
        ...
        flat_eos: List[str] = []
        for (bname, _btype, k, _bx, _by) in chain:
    ...
    fb_lines = []
    for p in plan:
        for (bname, btype, k, bx, by) in p["chain"]:
            # Format coordinates the same way the rest of the file uses them:
            # ints when possible, one decimal otherwise.
            xs = _fmt_coord(bx)
            ys = _fmt_coord(by)
            fb_lines.append(
                f'{INDENT_FB}<FB Name="{bname}" Type="{btype}" x="{xs}" y="{ys}">'
            )
