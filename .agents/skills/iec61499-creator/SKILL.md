...
**A plain `InputVar` used as a standalone configuration constant on a `.lib` building block (e.g. a timer's delay time) must not be left as a bare typed variable.** Give it a proper initialization path instead: declare it as a `Socket` of the matching adapter type (e.g. `adapter::types::unidirectional::ATM` for `TIME`, `AR` for `REAL`) and feed it, at the instantiation site, from the project's standard `initval_<TYPE>` block (e.g. `adapter::types::unidirectional::TIME::initval::initval_ATM`) with its `INIT`/`INITO` (`EInit`) events properly wired — the same `INIT`/`INITO` convention as section 7. Reference the field via `.D1` in `DataConnections` (e.g. `Source="DT.D1" Destination="E_DELAY.DT"`), exactly like an ordinary Socket's data field.
